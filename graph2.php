...
class graph {
  ...
  var $parameter         =   array(        // input parameters
	...
	'path_to_images'     => 'images/',     // path to image sub-directory
...
function draw_set($order, $set, $offset) {
  ...
  foreach ($this->x_data as $index => $x) {
    ...
    ////////////////echo "thisX=$thisX, thisY=$thisY, bar=$bar, barSize=$barSize, colour=$colour, offset=$offset, set=$set <br />";
...
/////////////////////////////		echo "y_data set index is=".$this->y_data[$set][$index]."<br />";
  ...
  if ($this->debug) { // for debugging purposes.
    ...
    if ($this->parameter['file_name'] == 'none' || empty($this->parameter['file_name'] )) {
      switch ($this->parameter['output_format']) {
       ...
       switch ($this->parameter['output_format']) {
        case 'GIF':
          ImageGIF($this->image, $this->parameter['path_to_images'].$this->parameter['file_name'].'.gif');
          break;
        case 'JPEG':
          ImageJPEG($this->image, $this->parameter['path_to_images'].$this->parameter['file_name'].'.jpg');
          break;
       default:
          ImagePNG($this->image, $this->parameter['path_to_images'].$this->parameter['file_name'].'.png');
          break;
      }
    }
